...
int main(int argc, char* argv[]) {
   ...
   // unsafe_parse_fen("4k3/p7/8/8/8/8/8/4K3 w - - 0 1", &board);
    ...
    int n_depth =12;
    printf("number of thread initialized: %ld\n",par_instance.num_search_threads);
    printf("position: r3k2r/p1ppqpb1/bn2pnp1/3PN3/1p2P3/2N2Q1p/PPPBBPPP/R3K2R w KQkq - 0 1\n");
...
//        search_task_t tasks[n_depth];
//        search_state_t search_states[n_depth];
//        board_t boards[n_depth];
//
...
//        printf("(%d) ", tasks[n_depth-1].search_state->pv_table[0][0]);
//        print_move(tasks[n_depth-1].search_state->pv_table[0][0], min);
...
//    }
    ...
    for (int i=0; i<4; i++) {
        ...
        move_t best_move = find_best_move(&board, 12, true, &cancel_flag);
