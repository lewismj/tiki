...
void print_move(move_t m, showable options) {
    ...
    if (options & min) {
        ...
        printf("%s:", piece_to_str[get_move_piece(m)]);
        printf("%s:", get_promoted_piece(m) == 0 ? "-" : piece_to_str[get_promoted_piece(m)]);
        printf("%s:", get_enpassant_flag(m) != 0 ? "t" : "f");
        printf("%s:", get_capture_flag(m) != 0 ? "t" : "f");
        printf("%s:", get_double_push_flag(m) != 0 ? "t" : "f");
        printf("%s:", get_king_side_castle_flag(m) != 0 ? "t" : "f");
        printf("%s\n", get_queen_side_castle_flag(m) != 0 ? "t" : "f");
    }
}