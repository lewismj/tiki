...
bitboard create_king_attack_mask(square s) {
    bitboard mask = 0ULL;
    bitboard loc = tiki_bitboard_const.square_mask[s];

    if (loc >> 8)  mask |= loc >> 8;                                    /* n.   */
    if (loc >> 9 & tiki_bitboard_const.not_h_file) mask |= loc >> 9;    /* nw.  */
    if (loc >> 7 & tiki_bitboard_const.not_a_file) mask |= loc >> 7;    /* ne.  */
    if (loc >> 1 & tiki_bitboard_const.not_h_file) mask |= loc >> 1;    /* w.   */

    if (loc << 8) mask |= loc << 8;                                     /* s.   */
    if (loc << 9 & tiki_bitboard_const.not_a_file) mask |= loc << 9;    /* sw.  */
    if (loc << 7 & tiki_bitboard_const.not_h_file) mask |= loc << 7;    /* se.  */
    if (loc << 1 & tiki_bitboard_const.not_a_file) mask |= loc << 1;    /* e.   */
...
bitboard create_knight_attack_mask(square s) {
    ...
    if (loc >> 17 & tiki_bitboard_const.not_h_file) mask |= loc >> 17;  /* nnw. */
    if (loc >> 15 & tiki_bitboard_const.not_a_file) mask |= loc >> 15;  /* nne. */
    if (loc >> 10 & tiki_bitboard_const.not_hg_file) mask |= loc >> 10; /* nw.  */
    if (loc >> 6 & tiki_bitboard_const.not_ab_file) mask |= loc >> 6;   /* ne.  */

    if (loc << 17  & tiki_bitboard_const.not_a_file) mask |= loc << 17; /* sse. */
    if (loc << 15 & tiki_bitboard_const.not_h_file) mask |= loc << 15;  /* ssw. */
    if (loc << 10 & tiki_bitboard_const.not_ab_file) mask |= loc << 10; /* se.  */
    if (loc << 6 & tiki_bitboard_const.not_hg_file) mask |= loc << 6;   /* sw.  */
...
bitboard create_pawn_attack_mask(square s, colour c) {
    ...
    if (c == white) {
        ...
        if (loc << 7 & tiki_bitboard_const.not_h_file) mask |= loc << 7;    /* sw.  */
        if (loc << 9 & tiki_bitboard_const.not_a_file) mask |= loc << 9;    /* se.  */
    }
...
bitboard set_occupancy_variation(unsigned int index, unsigned int num_relevant_bits, bitboard attack_mask) {
    ...
    for (int i=0; i < num_relevant_bits; i++) {
        ...
        if (index & tiki_bitboard_const.square_mask[i]) variation |= tiki_bitboard_const.square_mask[sq];
    }
